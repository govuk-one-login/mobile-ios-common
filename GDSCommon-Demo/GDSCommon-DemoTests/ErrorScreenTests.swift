...
extension GDSErrorScreenTests {
    ...
    func test_imageIsNeverNil() throws {
        // GIVEN we setup the SUT without providing an image
        let expectedDefaultImage = "exclamationmark.circle"
        
        // WHEN we initialise the SUT
        sut = createSUT()
        
        // THEN error image is still set as the default
        XCTAssertFalse(try sut.image.isHidden)
        XCTAssertEqual(sut.viewModel.errorDefaults.image, expectedDefaultImage)
    }
